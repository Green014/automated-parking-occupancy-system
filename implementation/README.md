...
The completed P3 workflow uses `parking-run-final` as the default research
entry point; `parking-run-integrated` remains an equivalent historical alias. It
...
```powershell
.\.venv\Scripts\parking-run-final.exe `
  --input <video> `
  --slots <slot-map.json> `
...
explicit runtime variants. The default command resolves
`configs/p3_stage_r_recommended_default_20260729.yaml`; variants are recorded
without changing that default method definition.
...
## 13. Stage V.1 corrected multi-backend runtime

Stage V.1 provides three core modes without changing Stage J–U.1 results:

- `classic`: independent OpenCV foreground-ratio teaching baseline;
- `detection`: frozen D1 + B1;
- `fusion`: frozen D1 + B1 + E1b + F2.

The default is Fusion. E4 and vehicle tracking are off by default. Frozen
C1/C2 configuration identity is hash- and parameter-checked; a changed
configuration requires `--allow-custom-config` and is recorded as custom.
The first frame no longer creates a false arrival, image directories create
no temporal events, and cache-hit FPS includes the attributed D1 cost.

See `data/STAGE_V_1_CLOSURE_REPORT.md` and run:

```powershell
.\.venv\Scripts\python.exe scripts\run_stage_v_multimode_demo.py `
  --input <video-or-image-directory> `
  --slots <slots.json> `
  --mode compare `
  --d1-weights <D1-best.pt> `
  --e1b-checkpoint <E1b-best.pt> `
  --output-dir outputs\stage_v_1_<new-id>
```

## 14. Stage W local Flask dashboard

Stage W adapts the member project's dashboard presentation to the unified
backend. It shows annotated MJPEG video, every configured polygon,
occupied/vacant/total counts, attributed FPS, mode/cache state, optional
temporal/tracker state and recent events. Browser refreshes reuse one
synchronized backend process.
...
Outputs use a new directory and include annotated video, status, events,
summary and configuration snapshot. Model files, raw datasets and credentials
are never committed. Missing models or input errors are explicit; no backend
is silently substituted.

The optional `member-reference` mode requires an external checkout at the
audited commit. Public redistribution of the adapted interface is recorded as
confirmed by the project owner; the evidence remains privately retained
outside the repository. See:

- `data/STAGE_W_UI_INTEGRATION_REPORT.md`;
- `data/PUBLIC_PERMISSION_AND_PROVENANCE.md`;
- `data/STAGE_W_REPRODUCTION_GUIDE.md`;
- `data/STAGE_V_1_W_RELEASE_INDEX.md`.

## 15. Stage W.1 release hardening

Stage W.1 changes source, dependency metadata, documentation and release
verification only. It does not load a model, regenerate predictions, rewrite
the four-frame dashboard demonstration or alter any reported metric.

The user-facing roles are deliberately separate:

- `parking-run-final`: default P3 research runtime;
- `parking-compare`: controlled Stage V.1 Classic/Detection/Fusion comparison;
- `parking-dashboard`: local Stage W dashboard;
- `scripts/run_p3_tt.py`: optional P3-TT TrackTrack experiment.

Install the complete local development/test environment with
`.[integrated,dashboard,dev]`. `stage_w_requirements.txt` remains a
compatibility-only Flask installation entry. See
`data/STAGE_W_1_RELEASE_INDEX.md`.

## 16. Stage W.2 cross-platform release packaging

Stage W.2 adds a repository-level `.gitattributes`, a machine-readable source
commit manifest, and a model-free temporary Git checkout verifier. Release
text is checked out with LF even when `core.autocrlf=true`; model files,
videos, images, documents and archives remain binary.

The formal P3 config remains byte-identical with SHA-256
`198f627689cd93f66ca0f087af6686d3afc697ff51e2aa77ee56124187b981b0`.
The source manifest excludes weights, datasets, outputs/runs, environments,
caches, machine-local path records, local vendor copies and unauthorized
member model/data/video assets without deleting any local file.

W.1 is retained as the exact pre-W.2 historical source snapshot. W.2 is the
pre-W.3 cross-platform historical snapshot. Its recorded test totals are not
rewritten by later environments. Verify the W.2 boundary without inference:

```powershell
python scripts\verify_stage_w_2_checkout.py --current-only
```

Run the isolated Windows line-ending simulation:

```powershell
python scripts\verify_stage_w_2_checkout.py
```

The latter copies only the source manifest candidates to the system temporary
directory, commits only inside that temporary repository, clones with
`core.autocrlf=true`, verifies the registry and frozen config, runs four CLI
`--help` checks and compiles source. It never operates on this repository's
index and never connects a public remote. See
`data/STAGE_W_2_RELEASE_INDEX.md`.

## 17. Stage W.3 privacy-safe public source and model release preparation

Stage W.3 adds the unmodified AGPL-3.0 license text, third-party notices, an
anonymous permission/provenance record, D1/E1b model cards, and separate
source/model release manifests. Both Python package metadata files declare
`AGPL-3.0-only`. The public source resolver excludes the older private
permission record, local user paths, email addresses, credential-bearing RTSP
strings, secrets, weights, datasets, outputs, vendor copies, environments,
and caches.
...
instead of skip, and TrackEval controls its marked tests. W.3 records the
actual current totals without changing W.2's report. The canonical W.3
validation uses the repository-relative
`.venv_stage_o_retinexformer/Scripts/python.exe` on Python 3.12.13, with Flask
present and TrackEval absent.

Verify the current worktree without models or inference:

```powershell
python scripts\verify_stage_w_3_release.py --current-only
```

Verify the privacy-safe source in a temporary `core.autocrlf=true` clone:
