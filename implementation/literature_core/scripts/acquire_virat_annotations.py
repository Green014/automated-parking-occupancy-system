"""Acquire exact official VIRAT annotation items without overwriting files."""

from __future__ import annotations

import argparse
import json
from pathlib import Path
import sys

import yaml

ROOT = Path(__file__).resolve().parents[1]
SRC = ROOT / "src"
if str(SRC) not in sys.path:
    sys.path.insert(0, str(SRC))

from literature_core.virat_access import (  # noqa: E402
    ANNOTATIONS_FOLDER_ID,
    ViratItem,
    download_item,
    fetch_folder_items,
    parse_annotation_item,
    select_named_items,
)


def _record(item: ViratItem) -> dict[str, object]:
    return {
        "item_id": item.item_id,
        "name": item.name,
        "size": item.size,
        "scene_id": item.scene_id,
        "sequence_id": item.sequence_id,
    }


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--names", nargs="+", required=True)
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("datasets/virat/screening/annotations"),
    )
    parser.add_argument("--budget-bytes", type=int, default=20 * 1024 * 1024)
    parser.add_argument("--max-item-bytes", type=int, default=15 * 1024 * 1024)
    parser.add_argument("--download", action="store_true")
    parser.add_argument(
        "--protocol-config",
        type=Path,
        default=Path("configs/temporal_protocol_pending.yaml"),
    )
    args = parser.parse_args()

    catalog = [
        item
        for raw in fetch_folder_items(ANNOTATIONS_FOLDER_ID)
        if (item := parse_annotation_item(raw)) is not None
    ]
    selected = select_named_items(
        catalog,
        args.names,
        max_total_bytes=args.budget_bytes,
        max_item_bytes=args.max_item_bytes,
    )
    payload: dict[str, object] = {
        "official_folder_id": ANNOTATIONS_FOLDER_ID,
        "budget_bytes": args.budget_bytes,
        "declared_total_bytes": sum(item.size for item in selected),
        "items": [_record(item) for item in selected],
    }
    if args.download:
        protocol = yaml.safe_load(args.protocol_config.read_text(encoding="utf-8"))
        acceptance = (
            protocol.get("dataset", {}).get("access", {}).get(
                "user_acceptance_recorded"
            )
            if isinstance(protocol, dict)
            else None
        )
        if acceptance is not True:
            parser.error(
                "download is blocked until protocol-config records individual "
                "license acceptance"
            )
        payload["acceptance_record"] = str(args.protocol_config)
        payload["downloads"] = [
            {"item": _record(item), **download_item(item, args.output_dir)}
            for item in selected
        ]
    print(json.dumps(payload, indent=2))


if __name__ == "__main__":
    main()
