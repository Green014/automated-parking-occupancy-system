...
def test_historical_registry_audit_excludes_current_stage_u_registry() -> None:
    _, repository_root = _roots()
    payload = audit_historical_registries(repository_root)
    ...
    if os.environ.get("STAGE_U_PORTABLE_PACKAGE") == "1":
        assert payload["registry_count"] == 2
        assert payload["local_only_count"] == 0
    else:
...
def test_portable_registry_has_relative_non_output_paths_when_generated() -> None:
    ...
    result = verify_portable_registry(
        registry,
...
def test_saved_audit_csv_and_registry_form_stable_one_way_chain() -> None:
    ...
    result = verify_saved_submission_evidence(
    ...
    assert result["verified"] is True
    assert result["candidate_file_count"] == result[
        "registry_candidate_file_count"
    ]
